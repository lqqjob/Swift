...
struct RankCellView: View {
    ...
    let item:AppRank
    @ObservedObject private var appModel = AppDetailModel()
    @AppStorage("kIsShowAppDataSize") private var isShowAppDataSize = false
    var body: some View {
        HStack {
            ImageLoaderView( url: item.imImage.last?.label) {
            ...
            VStack(alignment: .leading) {
                HStack(alignment: .top) {
                    Text("\(index + 1)").font(.system(size:16,weight: .heavy,design: .default))
                        .padding(EdgeInsets(top: 0, leading: 0, bottom: 0, trailing: 5))
                    
                    VStack(alignment: .leading) {
                        Text(item.imName.label)
                            .foregroundStyle(Color.tsmg_blue)
                            .font(.headline)
                            .lineLimit(2)
                            .truncationMode(.tail)
                        Spacer().frame(height: 5)
                        if isShowAppDataSize {
                            Text("占用大小：\(appModel.app?.fileSizeMB ?? "")").font(.footnote).lineLimit(1).foregroundStyle(.gray)
                            Text("最低支持系统：\(appModel.app?.minimumOsVersion ?? "")").font(.footnote).lineLimit(1).foregroundStyle(.gray)
                        }else {
                        ...
                        HStack {
                            ...
                            if item.imPrice.attributes.amount != "0.00" {
                                Text("\(item.imPrice.attributes.currency) \(item.imPrice.attributes.amount)").font(.footnote).foregroundStyle(.pink)
                            }
                        }.frame(height: 10)
                        Text(item.imArtist.label).font(.footnote).lineLimit(1).foregroundStyle(.gray)
