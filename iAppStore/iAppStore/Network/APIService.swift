...
//  iAppStore
//
//  Created by liqiang on 2025/2/7.
//
...
public struct APIService {
    ...
    public func POST<T:Codable>(endpoint:EndPoint,params:[String:String]?,completionHander:@escaping (Result<T,APIError>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { (data,response,error) in
            guard let data = data else {
                DispatchQueue.main.async {
                    completionHander(.failure(.noResponse))
                }
                return
            }
            ...
            guard error == nil else {
                DispatchQueue.main.async {
                    completionHander(.failure(.networkError(error: error!)))
                }
                return
            }
...
#endif
                    completionHander(.failure(.jsonDecodingError(error: error)))
                }
            }
    ...
    public func GET_JSON(endpoint:EndPoint,params:[String:String]?,completionHander:@escaping (Result<Dictionary<String,Any>,APIError>) -> Void) {
        let queryURL = endpoint.url()
